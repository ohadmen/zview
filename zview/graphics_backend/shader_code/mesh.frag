...
layout(location = 1) in vec3 v_xyz;
layout(location = 2) in vec3 v_eyeDir;
...
void main() {
    switch (pc.txt) {
        ...
        case 2: {
            ...
            vec3 light_dir = normalize(pc.lightDir.xyz + v_eyeDir);
        ...
        case 5: {
            ...
            vec3 ec_pos = v_xyz.xyz;
            vec3 ec_normal = normalize(cross(dFdx(ec_pos), dFdy(ec_pos)));
            vec3 reflect_dir = -reflect(pc.lightDir.xyz, ec_normal);
