...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData *draw_data,
  ...
  if (bd->HasClipOrigin) {
    ...
    glGetIntegerv(GL_CLIP_ORIGIN,
                  reinterpret_cast<GLint *>(&current_clip_origin));
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data) {
  ...
  glGetIntegerv(GL_TEXTURE_BINDING_2D,
                reinterpret_cast<GLint *>(&last_texture));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
  GLuint last_sampler;
  if (bd->GlVersion >= 330 || bd->GlProfileIsES3) {
  ...
  glGetIntegerv(GL_ARRAY_BUFFER_BINDING,
                reinterpret_cast<GLint *>(&last_array_buffer));
#ifndef IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY
  ...
  glGetIntegerv(GL_VERTEX_ARRAY_BINDING,
                reinterpret_cast<GLint *>(&last_vertex_array_object));
#endif
#ifdef IMGUI_IMPL_OPENGL_HAS_POLYGON_MODE
